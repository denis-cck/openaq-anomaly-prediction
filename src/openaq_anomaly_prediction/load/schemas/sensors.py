...
class SensorsTable(BaseTable):
    """Table management for the raw sensors table in BigQuery."""

    def __init__(self):
        super().__init__()

        self.dataset_id = SENSORS_TABLE_CONFIG["dataset_id"]
        self.table_id = SENSORS_TABLE_CONFIG["table_id"]

        self.schema = SENSORS_TABLE_CONFIG["schema"]
        self.partitioning = SENSORS_TABLE_CONFIG["partitioning"]
        self.clustering_fields = SENSORS_TABLE_CONFIG["clustering_fields"]
        self.primary_keys = SENSORS_TABLE_CONFIG["primary_keys"]
        self.foreign_keys = SENSORS_TABLE_CONFIG["foreign_keys"]
