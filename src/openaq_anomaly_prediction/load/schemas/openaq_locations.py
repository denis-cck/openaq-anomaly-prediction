...
OPENAQ_LOCATIONS_TABLE_CONFIG = {
    "dataset_id": "raw",
    "table_id": "src_openaq__locations",
    "schema": [
        # --------------------------------------------------------------------
        # RAW FIELDS
        ...
        bigquery.SchemaField("isMobile", "BOOL", mode="NULLABLE"),
        bigquery.SchemaField("isMonitor", "BOOL", mode="NULLABLE"),
        bigquery.SchemaField("distance", "FLOAT64", mode="NULLABLE"),  # in meters
        #
        # --------------------------------------------------------------------
        # FLATTENED FIELDS
        bigquery.SchemaField("bounds_min_lon", "FLOAT64", mode="NULLABLE"),
        bigquery.SchemaField("bounds_min_lat", "FLOAT64", mode="NULLABLE"),
        bigquery.SchemaField("bounds_max_lon", "FLOAT64", mode="NULLABLE"),
        bigquery.SchemaField("bounds_max_lat", "FLOAT64", mode="NULLABLE"),
        #
        ...
        #
        bigquery.SchemaField("coordinates_latitude", "FLOAT64", mode="NULLABLE"),
        bigquery.SchemaField("coordinates_longitude", "FLOAT64", mode="NULLABLE"),
        #
        bigquery.SchemaField("datetimeFirst_utc", "TIMESTAMP", mode="NULLABLE"),
        bigquery.SchemaField("datetimeFirst_local", "DATETIME", mode="NULLABLE"),
        bigquery.SchemaField("datetimeLast_utc", "TIMESTAMP", mode="NULLABLE"),
        bigquery.SchemaField("datetimeLast_local", "DATETIME", mode="NULLABLE"),
        #
        # --------------------------------------------------------------------
        # METADATA FIELDS
        ...
        # --------------------------------------------------------------------
        # REMOVED FIELDS
        # bigquery.SchemaField("instruments"),
        # bigquery.SchemaField("sensors"),
        # bigquery.SchemaField("licenses"),
        # bigquery.SchemaField("bounds"),  # replaced with separate columns
    ],
    "partitioning": None,
...
class OpenAQLocationsTable(BaseTable):
    """Table management for the raw locations table in BigQuery."""

    def __init__(self):
        super().__init__()

        self.dataset_id = OPENAQ_LOCATIONS_TABLE_CONFIG["dataset_id"]
        self.table_id = OPENAQ_LOCATIONS_TABLE_CONFIG["table_id"]

        self.schema = OPENAQ_LOCATIONS_TABLE_CONFIG["schema"]
        self.partitioning = OPENAQ_LOCATIONS_TABLE_CONFIG["partitioning"]
        self.clustering_fields = OPENAQ_LOCATIONS_TABLE_CONFIG["clustering_fields"]
        self.primary_keys = OPENAQ_LOCATIONS_TABLE_CONFIG["primary_keys"]
        self.foreign_keys = OPENAQ_LOCATIONS_TABLE_CONFIG["foreign_keys"]

    def clean_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        """Clean and sanitize the locations dataframe."""
        ...
        # cleaned_df["distance"] = cleaned_df["distance"].astype(float)
        ...
        cleaned_df[new_cols] = pd.DataFrame(
            cleaned_df["bounds"].tolist(), index=cleaned_df.index
        )

        cleaned_df = cleaned_df.drop(
            columns=["instruments", "licenses", "sensors", "bounds", "sensors_flat"],
            errors="ignore",
    ...
    def save_dataframe(self, df: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        if verbose >= 4:
            print()
            logger.debug(f"[BIGQUERY] Saving {len(df)} locations in Big Query...")

        # Clean the dataframe
        cleaned_df = self.clean_dataframe(df)

        # Upsert locations to BigQuery
        if skip_bq:
            if verbose >= 3:
            ...
            if verbose >= 3:
                logger.success(
                    f"Saved {len(df)} locations into [{self.get_full_table_id()}] in {exec_time(start_time, fmt=True)}.\n"
                )
