...
class MeasurementsTable(BaseTable):
    """Table management for the raw measurements table in BigQuery."""

    def __init__(self):
        super().__init__()

        self.dataset_id = MEASUREMENTS_TABLE_CONFIG["dataset_id"]
        self.table_id = MEASUREMENTS_TABLE_CONFIG["table_id"]

        self.schema = MEASUREMENTS_TABLE_CONFIG["schema"]
        self.partitioning = MEASUREMENTS_TABLE_CONFIG["partitioning"]
        self.clustering_fields = MEASUREMENTS_TABLE_CONFIG["clustering_fields"]
        self.primary_keys = MEASUREMENTS_TABLE_CONFIG["primary_keys"]
        self.foreign_keys = MEASUREMENTS_TABLE_CONFIG["foreign_keys"]
