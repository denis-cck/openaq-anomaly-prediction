...
try:
    ...
    for year in years:
        ...
        for i, trimester in enumerate(trimesters):
            ...
            for location in CITY_LOCATIONS.items():
                ...
                if i >= (len(CITY_LOCATIONS) - 1):
                    progress.print(
                        f"Downloaded {i + 1} weather data in {exec_time(start_time):.2f}s",
                        current_progress=i + 1,
                        total_progress=len(CITY_LOCATIONS),
                        prefix_msg=f"{i + 1}/{len(CITY_LOCATIONS)}",
                        last=True,
            ...
            logger.success(
                f"[{run_id_prefix.upper()}] Downloaded weather data for {len(CITY_LOCATIONS)} locations in {exec_time(start_time):.2f}s"
            )
            print()

        # CONCATENATE into a YEAR parquet file
        logger.debug(
            f"[{CITY_NAME.upper()}_{year}] Concatenating weather data for {year}..."
        )
        ...
        concatenated_filename = f"{CITY_NAME}_{year}_weather.int.parquet"
        ...
        logger.success(
            f"[{CITY_NAME.upper()}_{year}] Concatenated all weather data from {year} in data/export/{concatenated_filename}"
        )
        print()
    ...
    logger.debug(
        f"[{CITY_NAME.upper()}] Concatenating ALL weather data for {CITY_NAME}..."
    )

    parquet_files = get_parquet_filepaths(
        os.path.join(cfg.DATA_EXPORT_PATH), f"{CITY_NAME}_*_weather.int.parquet"
    )
    for file in parquet_files:
        logger.trace(f"data/export/{file.split('/')[-1]}")

    concatenated_filename = f"{CITY_NAME}_weather.int.parquet"
    ...
    logger.success(
        f"[{CITY_NAME.upper()}] Concatenated all weather data for {CITY_NAME} in data/export/{concatenated_filename}"
    )
